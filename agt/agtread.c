/*  agtread.c-- functions to read in AGT files and identify  */
/*              their version.                              */
...
descr_line *agt_read_descr(long start,long len)
{
  ...
  lines=rmalloc(sizeof(descr_line)*(len+1));
...
static void read_da2(fc_type fc)
{
  ...
  if (numroom<0) return;
  room_name=rmalloc(numroom*sizeof(long));
...
static void read_da3(fc_type fc)
{
  ...
  if (numnoun<0) return;
  noun_sdesc=rmalloc(numnoun*sizeof(long));
  noun_pos=rmalloc(numnoun*sizeof(long));
  ...
  for(i=0;i<numnoun;i++)
	...
	noun[i].related_name=0;
      seti(location);seti(weight);seti(size);
      seti(key);
/* All of following flags known to be valid except
   pullable, on, and win */
      setb(pushable);setb(pullable);setb(turnable);setb(playable);
      setb(readable);setb(on);setb(closable);setb(open);setb(lockable);
      setb(locked);setb(edible);setb(wearable);setb(drinkable);
      setb(poisonous);setb(movable);setb(light);setb(shootable);
      seti(num_shots);seti(points);
      if (noun_inside!=NULL) 
...
static void read_da4(fc_type fc)
{
  ...
  creat_sdesc=rmalloc(numcreat*sizeof(long));
...
static void read_da5(fc_type fc)
{
  ...
  if (aver>=AGT15F) cmd_ptr=rmalloc(sizeof(long)*last_cmd);
    else cmd_ptr=NULL;
  ...
  for(i=0;i<last_cmd;i++)
    {
      ...
      if (aver<AGT15F) {
	command[i].data=rmalloc(MAX_CMD_SIZE*sizeof(integer));
   ...
   i.e. Master's Edition and proto-ME games. */
{
  ...
  for(i=0;i<last_cmd;i++)
    if (cmd_ptr[i]>=2) {
      ...
      command[i].data=rmalloc(command[i].cmdsize*sizeof(integer));
...
static int compute_endcode(int ver)
/* Computes the correct endcode for a given gamefile version  */
{
  int i;

  for(i=0;FIX_LIST[ver][i].tnew!=-1;i++);
  return (FIX_LIST[ver][i].told-3);  /* -3 to get to EndCmd */
}
   ...
   token encoding scheme is being used. */
{
 ...
 /* Check for the special cases we know about */
  if (aver==AGT135) {
    if (endcode==compute_endcode(AGT182)) {aver=AGT182;return;}
    else if (endcode==compute_endcode(AGT118)) {aver=AGT118; return;}
  }
  if (aver==AGTME10)
    if (endcode==compute_endcode(AGTME10A)) {aver=AGTME10A;return;}
  if (aver==AGTMAST)
    if (endcode==compute_endcode(AGTME155)) {aver=AGTME155;return;}
...
static void build_cmd_table(void)
{
  ...
  cmd_table=rmalloc(sizeof(short)*topcmd);
...
static void badtokerr(char *s, int tok)
{
...
static void null_func(char *whatever)
{
	return;
}
...
static void read_line(genfile fd,char *xtypestr)
{
  if (bhold==0)
    { 
  ...
  if (debug_da1 && xtypestr!=NULL) 
    {
      rprintf("%s %4d:%s",xtypestr,linenum,linebuffer);
      if (bhold) rprintf("     *");
      writeln("");
    }
  bhold=0;
...
static void report(char *s,genfile fd)
{
	...
	noun inside information; this is used by agtout */
{
  ...
  if (aver<AGTME10) 
    { SL_TEXT=81;SL_NAME=SL_WORD=23;SL_ROOM=31;}
  else 
    { SL_TEXT=81;SL_NAME=SL_WORD=16;SL_ROOM=31;}
  ...
  objflag=NULL; objprop=NULL;
  attrtable=NULL; proptable=NULL;
  oflag_cnt=0; oprop_cnt=0;
  propstr=NULL; propstr_size=0;
  vartable=NULL;  flagtable=NULL; 
...
/* We do this all at once to avoid fragmentation; all of the following
  ...
  if (numglobal>0)
    globalnoun=rmalloc(numglobal*sizeof(word));

  if (aver<AGTME15 && aver!=AGT10) {
    question=rmalloc(MaxQuestion*sizeof(tline));
    answer=rmalloc(MaxQuestion*sizeof(tline));
  } 
  else if (aver>=AGTME15) {
    quest_ptr=rmalloc(MaxQuestion*sizeof(descr_ptr));
    ans_ptr=rmalloc(MaxQuestion*sizeof(descr_ptr));}
  msg_ptr=rmalloc((last_message)*sizeof(descr_ptr));

  if (maxroom>=first_room) {
    room=rmalloc((maxroom-first_room+1)*sizeof(room_rec)); 
    room_ptr=rmalloc((maxroom-first_room+1)*sizeof(descr_ptr));
    help_ptr=rmalloc((maxroom-first_room+1)*sizeof(descr_ptr));
    special_ptr=rmalloc((maxroom-first_room+1)*sizeof(descr_ptr));
    if (diag) room_inside=rmalloc((maxroom-first_room+1)*sizeof(integer));
  }
    
  if (maxnoun>=first_noun) {
    noun=rmalloc((maxnoun-first_noun+1)*sizeof(noun_rec));
    noun_ptr=rmalloc((maxnoun-first_noun+1)*sizeof(descr_ptr));
    push_ptr=rmalloc((maxnoun-first_noun+1)*sizeof(descr_ptr));
    pull_ptr=rmalloc((maxnoun-first_noun+1)*sizeof(descr_ptr));
    text_ptr=rmalloc((maxnoun-first_noun+1)*sizeof(descr_ptr));
    turn_ptr=rmalloc((maxnoun-first_noun+1)*sizeof(descr_ptr));
    play_ptr=rmalloc((maxnoun-first_noun+1)*sizeof(descr_ptr));
    if (diag) noun_inside=rmalloc((maxnoun-first_noun+1)*sizeof(integer));
  }

  if (maxcreat>=first_creat) {
    creature=rmalloc((maxcreat-first_creat+1)*sizeof(creat_rec));
    creat_ptr=rmalloc((maxcreat-first_creat+1)*sizeof(descr_ptr));
    ask_ptr=rmalloc((maxcreat-first_creat+1)*sizeof(descr_ptr));
    talk_ptr=rmalloc((maxcreat-first_creat+1)*sizeof(descr_ptr));
    if (diag) creat_inside=rmalloc((maxcreat-first_creat+1)*sizeof(integer));
  }

  if (aver>=AGTME10) {
    userstr=rmalloc(MAX_USTR*sizeof(tline));
    sub_name=rmalloc(MAX_SUB*sizeof(word));
  }
  command=rmalloc(sizeof(cmd_rec)*last_cmd);

  if (aver>=AGT15)
    err_ptr=rmalloc(NUM_ERR*sizeof(descr_ptr));
 ...
 /* From this point on can assume ME detected */
  ...
  if (aver>=AGTME10) {  /* 2 rbool */
    ...
    if (aver>=AGTME15) {
      ...
      else freeze_mode=readrbool(fda1); } /* FREEZE */ 
    ver=3;
  }
  ...
  if (aver>=AGT15)
    ...
    for(i=0;i<NUM_ERR;i++)
      { err_ptr[i].start=0;err_ptr[i].size=-1;}
  ...
  if (aver>=AGTME10)
	...
	maxpix=MAX_PIX;
      ...
      t_pictlist=rmalloc(sizeof(long)*maxpict);
      t_pixlist=rmalloc(sizeof(long)*maxpix);
      t_songlist=rmalloc(sizeof(long)*maxsong);      
      t_fontlist=rmalloc(sizeof(long)*maxfont);
...
{
  static_str=NULL;ss_end=ss_size=0;
  command=NULL; cmd_ptr=NULL; synlist=NULL;
  userstr=NULL; sub_name=NULL; globalnoun=NULL; err_ptr=NULL;
  ...
  room=NULL;noun=NULL;creature=NULL;command=NULL;
...
static void free_da1_stuff(void)
     ...
     /* after freeing it */
{
  rfree(static_str);ss_end=ss_size=0;
  rfree(userstr);rfree(sub_name);rfree(globalnoun);rfree(err_ptr);
  rfree(synlist);
  rfree(quest_ptr);rfree(ans_ptr);
  rfree(question);rfree(answer);
  rfree(msg_ptr);rfree(room_ptr);rfree(help_ptr);rfree(special_ptr);
  rfree(noun_ptr);rfree(push_ptr);rfree(pull_ptr);rfree(text_ptr);
  rfree(room_inside);rfree(noun_inside);rfree(creat_inside);
  rfree(turn_ptr);rfree(play_ptr);
  rfree(creat_ptr);rfree(ask_ptr);rfree(talk_ptr);
  rfree(t_pictlist);rfree(t_pixlist);rfree(t_songlist);rfree(t_fontlist);
  rfree(room);rfree(noun);rfree(creature);rfree(command);
  free_dict();
}
    ...
    program of some sort */
{
 ...
 /* Compute max_score if it isn't already computed */
  ...
  if (ss_end>0)
    static_str=rrealloc(static_str,sizeof(char)*ss_end);
 ...
 /* Now convert string handles into honest pointers */
  ...
  if (aver>=AGTME10) {
    pictlist=rmalloc(sizeof(filename)*maxpict);
    pixlist=rmalloc(sizeof(filename)*maxpix);
    songlist=rmalloc(sizeof(filename)*maxsong);      
    fontlist=rmalloc(sizeof(filename)*maxfont);
 ...
 /* Free the various temporary arrays */
  rfree(room_name);rfree(noun_sdesc);rfree(noun_pos);rfree(creat_sdesc);
  rfree(t_pictlist);rfree(t_pixlist);rfree(t_songlist);rfree(t_fontlist);
...
/* Reallocate questions and asnwers to only use the space that they need */
  if (!RAW_CMD_OUT && top_quest<MaxQuestion) {
    ...
    if (top_quest==0) 
      {rfree(question);rfree(answer);rfree(quest_ptr);rfree(ans_ptr);}
    else {
      if (question!=NULL) 
	question=rrealloc(question,top_quest*sizeof(tline));
      if (answer!=NULL)
	answer=rrealloc(answer,top_quest*sizeof(tline));
      if (quest_ptr!=NULL)
